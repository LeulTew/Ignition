...
def test_breakdown_goal(mock_generate):
    ...
    response = client.post("/breakdown", json={"goal": "Test Goal"})
    assert response.status_code == 200
    data = response.json()
    assert data["steps"] == ["Step 1", "Step 2"]
    assert data["complexity"] == 5
    mock_generate.assert_called_once_with("Test Goal")
...
def test_sub_breakdown_step(mock_generate_sub):
    ...
    response = client.post("/sub-breakdown", json={"step": "Test Step"})
    assert response.status_code == 200
    data = response.json()
    assert data["substeps"] == ["Sub 1", "Sub 2"]
    mock_generate_sub.assert_called_once_with("Test Step")
